...
extract_posterior_t3 <- lapply(weighted_posterior_t3, function(model){
  ...
  Q1 = str_trim(sub('.{8}', '', model[2,])) #get 2.5% quantile 
  Q3 = str_trim(sub('.{8}', '', model[5,])) #get 97.5% quantile
  data.frame(Parameter, Beta, Q1, Q3)[c(1:2,5:6),] #retain only the four predictors 
})

# Extract prior and posterior probabilities and BF inclusion for every model

get_BFincl_t3 <- lapply(bf_inclusion_t3, function(model){
  ...
  Posterior = c(1, model$p_posterior) #add intercept
  data.frame(Prior, Posterior, BFincl)
})

# Calculate mean parameters across imputed dataframes and put them in a dataframe 

mean_posterior_t3 <- data.frame(do.call(rbind, extract_posterior_t3), row.names = NULL) %>%
  cbind(data.frame(do.call(rbind, get_BFincl_t3), row.names = NULL)) %>%
  ...
  mutate(Parameter = recode_factor(Parameter, "mu" = "Intercept", 
                                   "ScreeLing_1_tot" = "Initial language score", 
                                   "stroke_size" = "Acute lesion volume",
                                   "volume_LHC" = "Acute left hippocampal volume")) %>%
  group_by(Parameter) %>% filter(!Parameter=="sig2") %>% summarise(Beta = qwraps2::mean_sd(Beta, denote_sd = "paren"), 
...
flextable(mean_posterior_t3) %>% 
  theme_vanilla %>% 
  set_table_properties(layout = "autofit") %>%
  mk_par(j = 2, part = "header", value = as_paragraph(as_i(("β")))) %>% 
  flextable::footnote(part="body", i=2:4, j=1, ref_symbols = "*", 
